...
export default function ReturningCheckbox({ date, label, checked, value, onChange }: Props) {
  return (
    <li>
      <input
        type="radio"
        id={`${date}_returning_${value}`}
        name={`${date}_returning`}
        value={value}
        checked={checked}
      ...
      />
      <label
        htmlFor={`${date}_returning_${value}`}
